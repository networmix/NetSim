...
def bundle_members(dev: DeviceState, po: str) -> list[EthernetNode]:
    return sorted(
        (
            n
            for n in dev.interfaces.values()
...
def _connected_rows(dev: DeviceState, af: int) -> tuple[Route, ...]:
...
def _neighbors(state: NetworkState, device: str) -> NeighborTable:
    from netsim.model.addressing import mask_for
            ...
            for host, plen in local_addrs:
                ...
                for phost, _ in peer_addrs:
                    if (phost & mask) == (host & mask):
                        entries[(name, phost)] = peer_l3.mac
            if af == IPV6:
                entries[(name, MacAddress(peer_l3.mac).link_local_int())] = peer_l3.mac
    return NeighborTable(PMap(sorted(entries.items())), PMap(sorted(peers.items())))
...
def derive_l3(
    state: NetworkState, now: float, targets: Iterable[str] | None = None
) -> NetworkState:
    names = (
        list(targets)
        if targets is not None
        else [d for d, _ in state.devices.sorted_items()]
    )
    devices = state.devices.builder()
    for device in names:
        ...
        # Connected and local rows.
        ribs = new_dev.ribs.builder()
        for af in AFS:
            rib = new_dev.ribs.get(af) or RibState.empty(af)
            rows = _connected_rows(new_dev, af)
            rib2 = rib_apply(
                rib, sync=(CONNECTED, tuple(r for r in rows if r.source == CONNECTED))
            )
            rib2 = rib_apply(
        ...
        # Neighbors and router id.
        neighbors = (
            _neighbors(dataclasses.replace(state, devices=devices.build()), device)
            if False
            else _neighbors(_with_device(state, device, new_dev), device)
        )
        if neighbors != new_dev.neighbors:
...
def bump_epochs(old: NetworkState, new: NetworkState) -> NetworkState:
    ...
    (idempotent across a transaction that bumped it itself)."""
