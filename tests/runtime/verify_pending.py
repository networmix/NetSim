...
to run the original carrier-debounce workload (which also measures derivations,
...
def carrier(n):
    # Same construction and timed region as the original verify_pending.py.
    net = Network()
...
def main():
    ...
    measure = carrier if args.carrier else isolated
    if not args.carrier:
        for _ in range(3):
            isolated(1000)
    for n in args.sizes:
